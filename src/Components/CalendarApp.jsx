...
export const CalendarApp = () => {
     ...
     useEffect(() => {
        ...
        if (stored) {
          try {
            ...
            const normalized = parsed.map(evt => ({
              ...evt,
              date: evt && evt.date ? new Date(evt.date) : new Date()
            }));
   ...
   useEffect(() => {
    ...
    try {
      const toStore = events.map(evt => ({
        ...evt,
        date: evt.date instanceof Date ? evt.date.toISOString() : evt.date
      }));
      localStorage.setItem("calendarEvents", JSON.stringify(toStore));
    } catch (e) {
    ...
    <div className="calendar-app">
        ...
        <div className="events">
            {showEventPopup && <div className="event-popup">
                ...
                <textarea placeholder="Enter text(Max 60 characters)" value={eventText} 
                 onChange={(e)=> {
            ...
            {events.map((event, index)=>( 
                ...
                <div className="event-date-wrapper">
                    <div className="event-date">{`${monthsOfYear[event.date.getMonth()]} ${event.date.getDate()},
